/** @module shared/composables/useStaggerEntry - 交错入场动画组合式函数，基于 IntersectionObserver 实现元素逐个淡入效果 */
import { getCurrentInstance, onMounted, onUnmounted } from 'vue'

/** 交错入场动画组合式函数，使匹配选择器的子元素在进入视口时依次淡入
 * @param selector - 需要添加入场动画的子元素 CSS 选择器
 * @param options - 配置项
 * @param options.threshold - IntersectionObserver 可见比例阈值，默认 0.08
 * @param options.rootMargin - IntersectionObserver 根元素边距，默认 '0px 0px -40px 0px'
 * @param options.staggerMs - 相邻元素动画延迟间隔（毫秒），默认 60
 * @param options.translateY - 入场纵向位移（像素），默认 12
 */
