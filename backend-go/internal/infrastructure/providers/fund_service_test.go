...
type fakeFundRepository struct {
	funds []funddomain.FundItem
}

func (r fakeFundRepository) ListFunds() []funddomain.FundItem {
	return append([]funddomain.FundItem(nil), r.funds...)
}

func (r fakeFundRepository) FindFund(code string) (funddomain.FundItem, bool) {
...
func mustSearchFunds(t *testing.T, service *FundService, request funddomain.FundSearchRequest) funddomain.FundSearchData {
	t.Helper()
...
func TestSearchSupportsFrontendSortOptions(t *testing.T) {
	service := NewFundService(fakeFundRepository{funds: []funddomain.FundItem{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := mustSearchFunds(t, service, funddomain.FundSearchRequest{
				Page:      1,
				Size:      10,
				SortBy:    tt.sortBy,
...
func TestSearchDefaultsToFrontendPageSize(t *testing.T) {
	...
	got := mustSearchFunds(t, service, funddomain.FundSearchRequest{Keyword: "华夏"})
...
func TestSearchRanksNamePrefixAboveNameContains(t *testing.T) {
	...
	got := mustSearchFunds(t, service, funddomain.FundSearchRequest{Keyword: "沪深300", Size: 20})
...
func TestSearchMatchesPinyinAliases(t *testing.T) {
	...
	got := mustSearchFunds(t, service, funddomain.FundSearchRequest{Keyword: "hxcz", Size: 20})
...
func TestRankingUsesAllFundsBeforeLimiting(t *testing.T) {
	funds := make([]funddomain.FundItem, 60)
	for i := range funds {
		funds[i] = funddomain.FundItem{
