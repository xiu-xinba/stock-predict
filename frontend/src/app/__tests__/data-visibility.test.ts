...
import WatchlistView from '@/features/watchlist/WatchlistView.vue'
import FundDetailView from '@/features/funds/FundDetailView.vue'
import StockDetailView from '@/features/stocks/StockDetailView.vue'

vi.mock('@/shared/composables/useStaggerEntry', () => ({
...
vi.mock('@/features/market/api/market', () => ({
  ...
  fetchIndexKline: vi.fn(async () => ({ code: 0, message: 'success', data: [] })),
  fetchIndexMinute: vi.fn(async () => ({ code: 0, message: 'success', data: [] })),
  fetchSectorRanking: vi.fn(async () => ({ code: 0, message: 'success', data: [] })),
  fetchNorthboundFlow: vi.fn(async () => ({ code: 0, message: 'success', data: null })),
}))

vi.mock('@/features/stocks/api/stocks', () => ({
...
vi.mock('@/features/funds/api/funds', () => ({
...
vi.mock('@/features/watchlist/api/watchlist', () => ({
  fetchWatchlistQuotes: vi.fn(async () => ({ code: 0, message: 'success', data: [] })),
  fetchStockWatchlistQuotes: vi.fn(async () => ({
    code: 0,
    message: 'success',
    data: {
      '600519': {
...
function createTestRouter(path: string) {
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/predict', component: { template: '<div>Predict</div>' } },
    ],
  })
  router.push(path)
  return router
