 ...
 * 全局刷新浮动按钮组件
 *
 * 根据当前路由自动调用对应 Store 的刷新方法，
 * 支持空闲自动收缩、加载旋转动画、快捷键（Alt+R）触发，
 * 以及刷新完成后的时间提示。
 */
...
import { useFundDetailStore } from '@/features/funds'
import { useStockDetailStore } from '@/features/stocks'
...
const isLoading = computed(() => {
  return (
    ...
    marketStore.loading ||
    fundDetailStore.loading ||
    stockDetailStore.loading
  )
})
...
function refresh() {
  ...
  if (path.startsWith('/watchlist')) {
    ...
    const code = Array.isArray(route.params.fundCode)
      ? route.params.fundCode[0]
      : route.params.fundCode
    if (code) {
    ...
    const code = Array.isArray(route.params.stockCode)
      ? route.params.stockCode[0]
...
<template>
  <div
    :class="[
      'refresh-fab-wrap',
      { snapped: isSnapped, 'snap-right': isRightEdge, 'snap-left': !isRightEdge },
    ]"
    :style="{
      ...positionStyle,
      '--fab-size': fabSize,
      '--fab-compact': compactSize,
      '--fab-opacity': opacity,
    }"
  ...
  >
    <transition name="fab-tooltip">
      <span v-if="showTooltip && lastRefresh" class="fab-tooltip"> 已刷新 {{ lastRefresh }} </span>
    </transition>
    ...
    >
      <svg class="fab-icon" viewBox="0 0 1024 1024" aria-hidden="true">
        <path
          fill="currentColor"
          d="M771.776 794.88A384 384 0 0 1 128 512h64a320 320 0 0 0 555.712 216.448H654.72a32 32 0 1 1 0-64h149.44a32 32 0 0 1 32 32v148.16a32 32 0 1 1-64 0v-50.048zM296.064 229.12A384 384 0 0 1 896 512h-64a320 320 0 0 0-555.712-216.448h72.832a32 32 0 0 1 0 64H199.04a32 32 0 0 1-32-32V179.52a32 32 0 0 1 64 0v49.6z"
        />
      </svg>
    </button>
  </div>
